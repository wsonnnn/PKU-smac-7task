...
class ResBlock(nn.Module):
    ...
    def __init__(self, 
        ...
        self.conv = nn.Sequential(nn.Conv2d(hidden_channel, hidden_channel, 3, 1, 1),
                                  nn.ReLU(),
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        y_norm = self.norm2(self.MLP(x_norm))
...
class RelationalModule(nn.Module):
    ...
    def __init__(self, n_features, n_heads=3, n_hidden=384, n_blocks=2, dropout = 0.0):
...
class SC2Net(nn.Module):
    ...
    def __init__(self, minimap_channel, screen_channel, nonspatial_dim, n_actions):
        ...
        self.mini_conv = nn.Sequential(ResBlock(minimap_channel, 32),
                                       ResBlock(32, 32),
                                       nn.MaxPool2d(kernel_size=2))

        self.screen_conv = nn.Sequential(ResBlock(screen_channel, 32),
                                         ResBlock(32, 32),
                                         nn.MaxPool2d(kernel_size=2))
        ...
        #TODO : (feature-wise max-pooling operation)
        #(B, 64)
