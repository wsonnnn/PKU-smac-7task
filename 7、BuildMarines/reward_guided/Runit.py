...
if test_game is True:
    ...
    for x in l:
        print(x)
    print("average reward {}".format(total_r/30))
else:
    for i in range(0, max_episode):
        ...
        for t in range(max_step):
            ...
            agent.memory.append((obs, base_act, next_state.reward + Barre + Marre + SCVre, last_action, spatial_args, nonspatial_args))
            ...
            if state.last():
                agent.save(i, state.observation["score_cumulative"][0])
