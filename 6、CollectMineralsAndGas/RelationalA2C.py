...
class A2CActor(nn.Module):
    def __init__(self, nonspatial_dim=576, hidden_dim=256, screen_channel=64):
        ...
        self.base_policy_logit = nn.Sequential(nn.Linear(nonspatial_dim, hidden_dim), nn.ReLU(), nn.Linear(256, n_actions), nn.Softmax(dim=-1))

        spatial_args = {"screen": 0, "minimap": 0, "screen2": 0}

        # calculate spatial_args dict
        for (key, value) in spatial_args.items():
            spatial_args[key] = nn.Sequential(nn.ConvTranspose2d(screen_channel, 16, 4, 2, 1), nn.ReLU(), nn.ConvTranspose2d(16, 16, 4, 2, 1), nn.ReLU(), nn.Conv2d(16, 1, 1, 1),
                                              nn.Upsample(scale_factor=2, mode='nearest'), Flatten(), nn.Softmax(dim=-1))
        ...
        for arg in actions.TYPES:
            if arg.name not in spatial_features:
                size = arg.sizes[0]
                nonspatial[arg.name] = nn.Sequential(nn.Linear(nonspatial_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, size), nn.Softmax(dim=-1))
    ...
    def loss_fn(self, minimaps, screens, players, last_actions, valid_actions, base_action, spatial_args, nonspatial_args, advantage):
...
class A2CCritic(nn.Module):
    def __init__(self, nonspatial_dim=576, hidden_dim=256):
        ...
        self.value_net = nn.Sequential(nn.Linear(nonspatial_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, 1))
    ...
    def loss_fn(self, minimaps, screens, player, last_action, n_minimap, n_screen, n_player, n_laction, rewards):
...
class ActorCritic(nn.Module):
    def __init__(self, minimap_channel, screen_channel, nonspatial, n_actions):
        ...
        self.base_policy_logit = nn.Sequential(nn.Linear(576, 256), nn.ReLU(), nn.Linear(256, n_actions), nn.Softmax(dim=-1))
        ...
        # calculate spatial_args dict
        for (key, value) in spatial_args.items():
            spatial_args[key] = nn.Sequential(nn.ConvTranspose2d(64, 16, 4, 2, 1), nn.ReLU(), nn.ConvTranspose2d(16, 16, 4, 2, 1), nn.ReLU(), nn.Conv2d(16, 1, 1, 1),
                                              nn.Upsample(scale_factor=2, mode='nearest'), Flatten(), nn.Softmax(dim=-1))
        ...
        for arg in actions.TYPES:
            if arg.name not in spatial_features:
                size = arg.sizes[0]
                nonspatial[arg.name] = nn.Sequential(nn.Linear(576, 256), nn.ReLU(), nn.Linear(256, size), nn.Softmax(dim=-1))
                mask = torch.FloatTensor(np.eye(size))
                temp = torch.cat([torch.zeros(size).view(1, -1), mask], dim=0).to(device)
                self.nonspatial_mask[arg.name] = temp
        ...
        self.value_net = nn.Sequential(nn.Linear(576, 256), nn.ReLU(), nn.Linear(256, 1))
