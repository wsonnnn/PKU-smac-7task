...
map_names = ["MoveToBeacon", "CollectMineralShards", "FindAndDegitfeatZerglings", "DefeatRoaches", "DefeatZerglingsAndBanelings", "CollectMineralsAndGas", "BuildMarines"]
max_episode = 15000
max_step = 20000
map_name = map_names[3]
FLAGS = flags.FLAGS
FLAGS(sys.argv)
flags.DEFINE_bool("render", False, "Whether to render with pygame.")
env = sc2_env.SC2Env(map_name=map_name, step_mul=16, visualize=False, game_steps_per_episode=15000, agent_interface_format=[agent_format], players=[sc2_env.Agent(sc2_env.Race.terran)])
...
class Agent(object):
    ...
    def select_action_cms(self, obs):
    ...
    def select_action(self, obs, valid_actions, train=False, target=False):
        ...
        if action_id != 12:
            ...
            roaches = _xy_locs(player_relative == _PLAYER_ENEMY)
            return FUNCTIONS.Attack_screen("now", roaches[np.argmax(np.array(roaches)[:, 1])])
        return action

    def select_action_or(self, obs, valid_actions, train=False, target=False):
...
if test_game is True:
        ...
        while True:
            ...
            if map_name == "DefeatRoaches":
                if obs[2][12] == 0:
                    ...
                    obs[2][12] = 1
                    #obs[2][7] = 1
            #actions = agent.select_action_dr(state)
    ...
    for i in range(151, max_episode):
        if (i % 5 == 0):
            print(i)
        state = env.reset()[0]
        num = 0
        ...
        if (i % TEST_FREQUENCY == 0):
                ...
                if map_name == "DefeatRoaches":
                    if obs[2][12] == 0:
                        ...
                        for x in range(573):
                            obs[2][x] = 0
                        obs[2][12] = 1
                actions = agent.select_action_or(obs, valid_actions=obs[2], train=False)
                next_state = env.step(actions=[actions])[0]
                next_obs = agent.get_observation(next_state)
                state = deepcopy(next_state)
                if state.last():
            ...
            if r_all >= 30:
                agent.save(i, r_all)
            print('episode: {} , copy_dr_total_reward_mask: {}'.format(i, r_all))