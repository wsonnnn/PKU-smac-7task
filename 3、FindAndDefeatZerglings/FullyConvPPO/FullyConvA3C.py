...
class ActorCritic(nn.Module):
    ...
    def forward(self, minimaps, screens, players, last_actions):
        ...
        if debug:
            print("nonspatial shape", nonspatial.shape)
    ...
    def choose_action(self, minimap, screen, player, last_action, valid_action, israndom=True):
        ...
        base_prob = torch.clamp(base_prob, EPS, 1.0)
        ...
        # print("masked_base_prob", masked_base_prob)
        assert not torch.isnan(masked_base_prob).any()
    ...
    def cross_entropy_loss(self, minimaps, screens, players, 
                                 last_actions, valid_actions, 
                                 base_action, spatial_args, nonspatial_args, rewards,
                                 next_minimaps, next_screens, next_players, next_lactions):
    ...
    def loss_fn(self, minimaps, screens, players, 
        ...
        # obs, base_act, next_state.reward, last_action, next_obs, spatial_args, nonspatial_args
        # a batch is [trace1, trace2, trace3, ... trace_m], m is batch_size, we let m = 8
        batch_size = len(base_action)
        base_prob, spatial_prob, nonspatial_prob, values= self.forward(minimaps, screens, players, last_actions)

        with torch.no_grad():
            base_, spa_, non_, next_values = self.forward(next_minimaps, next_screens, next_players, next_lactions)

        '''
        batch_q = self._get_q_value(rewards, boostrapping.item())
        batch_q = torch.FloatTensor(batch_q).to(device)
        '''
        # calculate gae for policy
        '''
        new_rewards = torch.FloatTensor(rewards).to(device)
        adv = new_rewards.view(-1, 1) + self.gamma * next_values - values
        gae_adv = self._get_gae(adv)
        '''

        # calculate advantage for critic
        batch_q = self._get_q_value(rewards, next_values[-1].detach())
        # print(batch_q)
        ...
        if self.ENTROPY_SCALE > 1e-7:
            self.ENTROPY_SCALE -= (ENTROPY_SCALE - 1e-7)/self.decay_count
        loss = (actor_loss + BASELINE_SCALE * critic_loss + self.ENTROPY_SCALE * entropy_loss).mean()
        return loss
    ...
    def _get_gae(self, advantages):
        ...
        gae_lst.reverse()
        gae_lst = torch.FloatTensor(gae_lst).to(device)
        return gae_lst
...
class Worker(object):
    ...
    def save(self, i, r):
        torch.save(self.gnet.state_dict(), "modelsA3C/sc2_actorcritic_{}_{}_{:.4f}.pth".format(self.map_name, int(i), r))

    def load(self, i, r):
        self.gnet.load_state_dict(torch.load("models2A3C/sc2_actorcritic_{}_{}_{:.4f}.pth".format(self.map_name, int(i), r)))
    
    def obs_to_features(self, states):

        minimap = torch.FloatTensor(np.stack([x[0] for x in states], axis=0)).to(device)
        screen = torch.FloatTensor(np.stack([x[1] for x in states], axis=0)).to(device)
        valid_action = torch.FloatTensor(np.stack([x[2] for x in states], axis=0)).to(device)
        player = torch.FloatTensor(np.log(1 + np.stack([x[3] for x in states], axis=0))).to(device)
        return minimap, screen, valid_action, player


    def learn(self, next_minimap, next_screen, next_player, next_laction):

        # obs, base_act, next_state.reward, last_action, next_obs, (done?), spatial_args, nonspatial_args
        ...
        for x in batch:
            ...
            next_states.append(x[4])
            spatial_args.append(x[5])
            nonspatial_args.append(x[6])
        ...
        '''
        minimap, screen, valid_action, player = self.obs_to_features(state)
        next_minimaps, next_screens, next_vactions, next_players = self.obs_to_features(next_states)

        laction = torch.LongTensor(last_action).squeeze(-1).to(device)

        next_lactions = last_action[1:]
        next_lactions.append([next_laction.item()])

        next_lactions = torch.LongTensor(next_lactions).squeeze(-1).to(device)
        ...
        loss = self.lnet.loss_fn(minimap, screen, player, laction, valid_action, 
    ...
    def run(self):
        ...
        while self.global_nep.value < self.max_step:
            ...
            for t in range(20000):
                ...
                self.memory.append((obs, base_act, next_state.reward, last_action, next_obs, spatial_args, nonspatial_args))
