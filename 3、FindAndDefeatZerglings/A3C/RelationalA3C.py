...
class Worker(object):
    ...
    def save(self, i, r):
        torch.save(self.gnet.state_dict(), "models2/sc2_actorcritic_{}_{}_{:.4f}.pth".format(self.map_name, int(i), r))

    def load(self, i, r):
        self.gnet.load_state_dict(torch.load("models2/sc2_actorcritic_{}_{}_{:.4f}.pth".format(self.map_name, int(i), r)))
        ...
        while self.global_nep.value < self.max_step:
            ...
            for t in range(20000):
                ...
                if next_state.reward == 1:
                    ...
                    flag +=1
                t_r += next_state.reward
                #print(next_obs[1][1].shape)
                for i in range(64):
                    for j in range(64):
                        #print(next_obs[1][1][i][j],end = " ")
                        #print(next_obs[1][1][i][j])
                        #print(a[i][j])
                        if next_obs[1][1][i][j] != a[i][j]:
                            b[i][j] = 0
                        else:
                            b[i][j] += 1
                    #print("")
                ...
                if t_r <= 20:
                    ...
                    add_reward = (math.exp(t_r-20)*10)*next_state.reward
                    if flag > 8:
                        distance *= (t_r - 20) * 140
                    else:
                        distance = 0
                print("distance=",distance," flag = ",flag,"  add_r = ",add_reward)
                ...
                self.memory.append((obs, base_act, next_state.reward - distance + add_reward, last_action, spatial_args, nonspatial_args))
